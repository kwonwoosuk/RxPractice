    ...
    private func bind() {
        ...
        items
            .bind(to: tableView.rx.items(cellIdentifier: PersonTableViewCell.identifier, cellType: PersonTableViewCell.self)) { (row, element , cell) in
                ...
                //                let url = URL(string: self.sampleUsers[row].profileImage)
                //                cell.profileImageView.kf.setImage(with: url)
                //                cell.usernameLabel.text = self.sampleUsers[row].name
                //
                //                cell.detailButton.rx.tap
                //                    .bind(with: self) { owner, _ in
                //                        print("더보기 눌림")
                //                        let vc = DetailViewController()
                //                        vc.navigationItem.title = owner.sampleUsers[row].name
                //                        self.navigationController?.pushViewController(vc, animated: true)
                //                    }
                //                    .disposed(by: cell.disposeBag) // cell 의 디스포스 사용해야지 ~ 몽총아~
                
                
                let url = URL(string: element.profileImage)  // element 사용해야 생각한대로 작동해준다 element는 장식이야?? 뭐하러 만들어 놨어?
                cell.profileImageView.kf.setImage(with: url)
        ...
        searchBar.rx.searchButtonClicked
            ...
            .bind(with: self) { owner, text in
                ...
                if trimText.isEmpty {// 비어있으면 전부 다 보여주고
                    owner.items.onNext(owner.sampleUsers)
                } else {
                    let filtered = owner.sampleUsers.filter { person in
                        person.name.lowercased().contains(trimText.lowercased())
                    }
                    owner.items.onNext(filtered)
                }
            }
            .disposed(by: disposeBag)
        ...
        tableView.rx.modelSelected(Person.self)
            .bind(with: self) { owner, person in
                ...
                owner.selectedUsers.onNext(owner.selectedPeople)
            }
            .disposed(by: disposeBag)
