...
import RxCocoa
import SnapKit
//MVVM + RxSwift + Input/Output
class Homework2ViewController: UIViewController {
    
    private let tableView: UITableView = {
        let view = UITableView()
        
        view.register(HomeworkTableViewCell.self, forCellReuseIdentifier: HomeworkTableViewCell.identifier)
    ...
    func bind() {
        items
            .bind(to: tableView.rx.items(cellIdentifier: HomeworkTableViewCell.identifier, cellType: HomeworkTableViewCell.self)) { (row, element, cell ) in
